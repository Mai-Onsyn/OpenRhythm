...
object Global {
    ...
    init {
        ...
        settings.enabledMidiInputDeviceList.toSet().let {   // 去重
        ...
        settings.enabledMidiInputDeviceList.forEach { name ->
            ...
            ).apply { targetChannel = settings.MidiInteractionChannel }.also { Logger.i { "Opened MIDI input device: Virtual Keyboard" } }
            else try {
                portList.find { it.name == name }?.let {
                    midiInputDevices[it.id] = KtMidiInputDevice(it, midiAccess)
                    Logger.i { "Opened MIDI input device: ${it.name}" }
                } ?: {
                    Logger.w { "No output device found for $name" }
                    settings.enabledMidiInputDeviceList.remove(name)
                }
            } catch (e: Exception) {
                Logger.e(e) { "Error while configure midi input: $name" }
                settings.enabledMidiInputDeviceList.remove(name)
            }
        }
    }
...
private fun createMidiPlayer(access: MidiAccess): MidiPlayer2 {
    ...
    if (outputs.isEmpty()) {
        println("No available MIDI output device found")
    ...
    return MidiPlayer2(runBlocking {
        try {
            ...
            Logger.i { "Opened MIDI output device: ${Global.settings.SelectedOutputDeviceName}" }
            output
        } catch (e: Exception) {
            Logger.e(e) { "Cannot open MIDI output device: ${Global.settings.SelectedOutputDeviceName}" }
            null
        }
    }).also { it.init() }
}
